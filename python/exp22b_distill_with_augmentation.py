"""
Experiment 22b: Distill Wide Transformer to RVQ (WITH AUGMENTATION)

MOTIVATION:
===========
Exp 22 FAILED because the teacher only reached R²=0.750, not 0.806.

ROOT CAUSE ANALYSIS:
- Exp 21b's sweep used augment=True for all configs
- "Wider (384, 6L)" achieved 0.8064 WITH augmentation
- Exp 22 trained teacher WITHOUT augmentation → only 0.750

FIX: Add data augmentation to teacher training, matching Exp 21b.

Expected:
- Teacher: ~0.806 (matching Exp 21b)
- Student: ~0.80+ (if distillation works)
"""
...
def train_phase1_teacher(
    ...
    """Phase 1: Train Wide Transformer teacher WITH AUGMENTATION."""
    print("\n" + "="*60)
    print("PHASE 1: Training Wide Transformer Teacher (with augmentation)")
    print("="*60)
...
def main():
    print("="*70)
    print("EXPERIMENT 22b: Distill Wide Transformer to RVQ (WITH AUGMENTATION)")
    print("="*70)
    print()
    print("FIX: Exp 22 failed because teacher was trained WITHOUT augmentation.")
    print("     Exp 21b sweep used augment=True → teacher got 0.806")
    print("     Exp 22 used augment=False → teacher only got 0.750")
    print()
    ...
    # Use non-augmented dataset for LSTM
    ...
    # Phase 1: Train Teacher (with augmentation)
    ...
    if student_r2 > best_lstm_r2:
        ...
        print(f"\n   ⚠️ Student beats previous best (0.784) but not LSTM")
    else:
    ...
    print("   Exp 22 (teacher 0.750 → student): R² = 0.741 (no augment!)")
    print(f"   Exp 22b (teacher {teacher_r2:.3f} → student): R² = {student_r2:.4f}")
    
    # Save results
    results = {
        "experiment": "22b",
        "description": "Distill Wide Transformer to RVQ (with augmentation)",
        "fix": "Added data augmentation to match Exp 21b",
    ...
    results_path = RESULTS_DIR / "exp22b_distill_with_augmentation.json"
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {results_path}")
    
    # Save model
    model_path = MODELS_DIR / "exp22b_distilled_rvq.pt"
